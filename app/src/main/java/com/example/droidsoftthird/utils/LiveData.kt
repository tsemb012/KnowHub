...
fun <T: Any> LiveData<T>.observe(owner: LifecycleOwner, callback: (T) -> Unit) = observe(owner, Observer(callback))
...
inline fun <T : Any, LIVE1 : Any, LIVE2 : Any, LIVE3 : Any, LIVE4: Any, LIVE5: Any, LIVE6: Any> combine(
    ...
    return MediatorLiveData<T>().apply {
        value = initialValue
        listOf(liveData1, liveData2, liveData3, liveData4, liveData5, liveData6).forEach { liveData ->
            addSource(liveData) {
                val currentValue = value
                val liveData1Value = liveData1.value
                val liveData2Value = liveData2.value
                ...
                if (currentValue != null && liveData1Value != null &&
                    ...
                    value = block(currentValue, liveData1Value, liveData2Value, liveData3Value, liveData4Value, liveData5Value, liveData6Value)
                }
            }
        }
    }.distinctUntilChanged()
