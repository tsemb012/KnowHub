...
@AndroidEntryPoint
class ScheduleHomeFragment: Fragment(R.layout.fragment_schedule_registered) {
    ...
    private fun setupWeekLabel() {
        binding.dayOfWeekLabel.dayOfWeekLabel.children.forEachIndexed { index, view ->
            ...
            textView.text = daysOfWeekFromLocale()[index].getDisplayName(TextStyle.SHORT, Locale.ENGLISH).toUpperCase(
                Locale.ENGLISH)
    ...
    private fun setupCalendarMatrix() {
        ...
        binding.calendarMatrix.apply {
            ...
            monthScrollListener = this@ScheduleHomeFragment::scrollMonth //スクロール時の処理を入れてあげる。
        }
    }

    private fun setupEventList() {
        binding.recyclerView.apply {
            adapter = this@ScheduleHomeFragment.adapter
            layoutManager = LinearLayoutManager(context)
            addItemDecoration(DividerItemDecoration(context, LinearLayoutManager.VERTICAL).apply {
                AppCompatResources.getDrawable(context, R.drawable.divider)?.let { setDrawable(it) } ?: throw IllegalStateException("Divider drawable not found")
            })
        }
    }
...
class DayViewBinder(private val viewModel: ScheduleViewModel) :
    DayBinder<DayViewBinder.DayViewHolder> {
