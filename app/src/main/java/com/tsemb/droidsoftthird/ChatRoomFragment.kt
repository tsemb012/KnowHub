...
import com.tsemb.droidsoftthird.model.domain_model.fire_model.FireMyImage
    ...
    private val requestAudioPermissionLauncher = registerForActivityResult(ActivityResultContracts.RequestPermission()) { isGranted: Boolean ->
        if (isGranted) {
            binding.root.showSnackbar(
                R.string.audio_permission_granted, Snackbar.LENGTH_INDEFINITE,
                R.string.yes
            ) {//パーミッションが許可されたとき。
                startRecording()
            }
            ...
            binding.root.showSnackbar(
                R.string.audio_permission_denied, Snackbar.LENGTH_SHORT,
                R.string.yes
            ) //Permissionが拒否されたとき。
        }
    }
